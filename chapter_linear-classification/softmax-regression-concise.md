...
:begin_tab:`mxnet`
Même si l'entrée `X` est un tenseur de quatrième ordre, 
la couche `Dense` intégrée 
convertira automatiquement `X` en un tenseur de second ordre 
en gardant la dimensionnalité le long du premier axe inchangée.
:end_tab:

:begin_tab:`pytorch`
Nous utilisons une couche `Flatten` pour convertir le tenseur de quatrième ordre `X` en second ordre 
en gardant la dimensionnalité le long du premier axe inchangée.

:end_tab:

:begin_tab:`tensorflow`
Nous utilisons une couche `Flatten` pour convertir le tenseur de quatrième ordre `X` 
en gardant la dimension le long du premier axe inchangée.
:end_tab:

:begin_tab:`jax`
Flax permet aux utilisateurs d'écrire la classe du réseau de manière plus compacte
en utilisant le décorateur `@nn.compact`. Avec `@nn.compact`, on
peut simplement écrire toute la logique du réseau à l'intérieur d'une seule méthode de "passe avant" (forward pass), sans avoir besoin de définir la méthode `setup` standard dans la dataclass.
:end_tab:
