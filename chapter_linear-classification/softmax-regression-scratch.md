...
Maintenant, nous pouvons (**mettre en œuvre la fonction de perte d'entropie croisée**) en faisant la moyenne sur les logarithmes des probabilités sélectionnées.
:end_tab:

:begin_tab:`jax`
Maintenant, nous pouvons (**mettre en œuvre la fonction de perte d'entropie croisée**) en faisant la moyenne sur les logarithmes des probabilités sélectionnées.

Notez que pour utiliser `jax.jit` afin d'accélérer les mises en œuvre JAX, et
pour s'assurer que `loss` est une fonction pure, la fonction `cross_entropy` est redéfinie
à l'intérieur de `loss` pour éviter l'utilisation de variables ou fonctions globales
qui pourraient rendre la fonction `loss` impure.
Nous renvoyons les lecteurs intéressés à la [documentation JAX](https://jax.readthedocs.io/en/latest/notebooks/Common_Gotchas_in_JAX.html#pure-functions) sur `jax.jit` et les fonctions pures.
:end_tab:
